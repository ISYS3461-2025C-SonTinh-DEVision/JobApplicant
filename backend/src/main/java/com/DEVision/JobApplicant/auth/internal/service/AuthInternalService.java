...
public class AuthInternalService {
    ...
    public Map<String, Object> resendActivationEmail(String email) {
        ...
        // Rate limiting: Check if token was recently generated (within 60 seconds)
        LocalDateTime tokenExpiry = user.getActivationTokenExpiry();
        if (tokenExpiry != null) {
            // Token expiry is set to 24 hours from creation, so we can calculate when it was created
            LocalDateTime tokenCreatedAt = tokenExpiry.minusHours(24);
            LocalDateTime minResendTime = tokenCreatedAt.plusSeconds(60);
            
            if (LocalDateTime.now().isBefore(minResendTime)) {
                long secondsRemaining = java.time.Duration.between(LocalDateTime.now(), minResendTime).getSeconds();
                return Map.of(
