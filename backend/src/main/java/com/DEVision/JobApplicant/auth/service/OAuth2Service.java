...
/**
 * Service to handle OAuth2 authentication with Google using ID Token Flow
 *
 * This service implements the ID Token Flow (Client-Side Flow):
 * - Frontend obtains Google ID token using Google's JavaScript SDK
 * - Backend verifies the token with Google's tokeninfo endpoint
 * - Backend extracts user information from the verified token
 *
 * No Authorization Code Flow implementation (simpler, frontend-controlled)
 */
@Service
public class OAuth2Service {
    ...
    /**
     * Verify Google OAuth2 ID token - Simplified for ID Token Flow
     *
     * This method wraps verifyGoogleIdToken() to maintain compatibility with
     * the existing service layer interface. Since we only support Google SSO,
     * the provider parameter is ignored.
     *
     * @param idToken The Google ID token from frontend
     * @param provider The OAuth2 provider (currently only "google" supported)
     * @return OAuth2UserInfo containing user details from Google
     * @throws Exception if token verification fails
     */
    public OAuth2UserInfo verifyToken(String idToken, String provider) throws Exception {
        // Only Google is supported - provider parameter ignored for simplicity
