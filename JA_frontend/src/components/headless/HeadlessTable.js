 ...
 * 
 * Usage:
 * const { sortKey, direction, toggleSort, sortedData, selectedRows } = useHeadlessTable({
 *   data: [...],
 *   defaultSortKey: 'name',
 * });
 */
...
export default function useHeadlessTable({
  data = [],
  defaultSortKey = null,
  ...
  const getRowProps = useCallback((id) => ({
    ...
    onClick: selectable ? () => selectRow(id) : undefined,
  }), [selectable, expandable, isSelected, isExpanded, selectRow]);
