 ...
 * - Resend activation with rate limiting (60 seconds cooldown)
 */
...
import {
    ...
    RefreshCw, PartyPopper, Sparkles, AlertTriangle, LogIn, Clock
} from 'lucide-react';
...
export default function VerifyEmailPage() {
    ...
    const handleResendActivation = async () => {
        ...
        try {
            const result = await authService.resendActivationEmail(resendEmail);

            if (result.success) {
                if (result.alreadyActivated) {
                    setResendMessage('✅ This account is already activated. You can login now!');
                } else {
                    setResendMessage('✅ A new activation link has been sent to your email!');
                    setResendCooldown(RESEND_COOLDOWN_SECONDS);
                }
            } else if (result.rateLimited) {
                setResendCooldown(result.retryAfter || RESEND_COOLDOWN_SECONDS);
                setResendMessage(`⏳ Please wait ${result.retryAfter || RESEND_COOLDOWN_SECONDS} seconds before requesting again.`);
            } else {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (errorMessage.includes('expired')) {
                    ...
                    setMessage('Your activation link has expired. Please request a new one below.');
    ...
    // Render expired state with resend option
    const renderExpired = () => (
        <div className="text-center animate-fade-in">
            ...
            <div className="bg-dark-700/50 rounded-xl p-4 mb-6">
                ...
                {resendMessage && (
                    <p className={`mt-3 text-sm ${resendMessage.includes('✅') ? 'text-green-400' : resendMessage.includes('❌') ? 'text-red-400' : 'text-amber-400'}`}>
                        {resendMessage}
                    </p>
                )}
            </div>
    ...
    // Render no token state with resend option
    const renderNoToken = () => (
        <div className="text-center animate-fade-in">
            <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-amber-500/20 flex items-center justify-center">
